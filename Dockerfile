...
#&& pip install git+https://github.com/apache/incubator-airflow.git@${AIRFLOW_VERSION}#egg=apache-airflow[async,crypto,celery,kubernetes,jdbc,password,postgres,hive,s3,slack] \
...
RUN set -ex \
    ...
    && pip install pyasn1 \
    && pip install apache-airflow[crypto,celery,postgres,password,hive,jdbc,s3,slack,redis,mysql,ssh${AIRFLOW_DEPS:+,}${AIRFLOW_DEPS}]==${AIRFLOW_VERSION} \
